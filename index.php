...
<?php if (isset($bruker)): ?>
    ...
    <p class="one">Hei <?= htmlspecialchars($bruker["name"]) ?>!</p>
    ...
    if($is_admin){
        ...
        $sql = "CALL getUserSubjects({$_SESSION["s_bruker_id"]});";
        $rows = $mysqli->query($sql);
        $emner = mysqli_fetch_all($rows, MYSQLI_ASSOC);
        $mysqli -> next_result();
    }
    ...
    // Dersom bruker ikke er admin, vil de få listet opp sin rolle vanlig    
        ...
        $sql = 
        "CALL getUserRoles({$_SESSION["s_bruker_id"]});";

        $role_result = $mysqli->query($sql);
        $mysqli -> next_result();
